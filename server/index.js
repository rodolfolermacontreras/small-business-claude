...
import { connectors, connectorStatus } from "./connectors/index.js";
import { optimizeAll } from "./optimizer.js";
import { loadSession, saveSession, deleteSession, listOutbox, getOutboxItem, updateOutboxItem } from "./db.js";
...
app.get("/api/outbox", (req, res) => res.json(listOutbox()));
...
app.post("/api/outbox/:id/approve", (req, res) => {
  const item = getOutboxItem(req.params.id);
...
app.post("/api/chat", async (req, res) => {
  ...
  const convo = loadSession(sessionId);
  convo.push({ role: "user", content: userText });

  try {
    const { text, steps, messages } = await runAgent(convo);
    saveSession(sessionId, messages);
    res.json({ reply: text, steps, outbox: listOutbox() });
  } catch (err) {
